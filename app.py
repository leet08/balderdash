...
def create():
    ...
    if request.method == 'POST':
        ...
        # shuffle
        newQuestions = Question.query.order_by(func.random())[:numberQuestions]
        for i in range(numberQuestions):
            Db.session.add(QuestionsInUse(question = newQuestions[i].question, correct = newQuestions[i].correct, gid = newGame.gid))
...
def enter():
    ...
    if request.method == 'POST':
        ...
        if userFlag == False and username != 'ADMIN':
            ...
            # assign to session uid if user hasn't been in before
